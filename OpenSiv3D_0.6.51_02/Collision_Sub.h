...
//
//
//void SphereBoxBlock(const Vec3& oldpos, Sphere* sphere, const OrientedBox* box)
//{
...
//		CalulateDivisionToHit_Sphere_Box(OldPos, 0, divnum, oneValue, sphere, box, &hitdivnum, &hitpos);
//
//		// どの面と衝突しているかを計算し、その法線ベクトルを取得する
//		// 計算用に衝突した瞬間のSphereを作成
//		Sphere MomentHitSphere{};
//		MomentHitSphere.set(*sphere);
//		MomentHitSphere.setPos(hitpos);
//
//		// 衝突している一番近い面の法線、衝突している面の数、最接近点(Box側)を取得する
//		Array<Vec3> SurfaceNormal; int hitnum; Vec3 cp;
//		Sphere_Box_GetHitSurfaceNormal(&MomentHitSphere, box, &SurfaceNormal, &hitnum, &cp);
//
//		// 残りの移動をさせる処理
//		if (hitnum == 1)
//		{
